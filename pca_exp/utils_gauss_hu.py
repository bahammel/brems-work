...
def gauss_data_2():
    ...
    plt.figure()
    plt.title('data')
    plt.plot(hu,np.transpose(xtrain[::200]))
...
def gauss_data():
    ...
    for gauss in range(num_of_gauss):
        mu = np.random.choice(np.linspace(2, 9, 20))
        std = np.random.choice(np.linspace(.05, 3.0, 20))
        #amp = (10. - hu) * np.random.random(1); #amp;
        #amp = np.random.random(1); amp;
        amp = np.random.choice(np.linspace(0.1, 1.0, 20))
        #amp = 1.0
        #I = I + amp * 1./(std*np.sqrt(2.*np.pi)) * np.exp(-(hu - mu)**2./(2.*std**2.))
        #I = (1. - hu/10000.) / (std*np.sqrt(2.*np.pi)) * np.exp(-(hu - mu)**2./(2.*std**2.))
        I = amp * (1.0 - hu/10.5) / (std*np.sqrt(2.*np.pi)) * np.exp(-(hu - mu)**2./(2.*std**2.))
        Y.append([mu,std,amp])
        #X.append([I, hu]) 
        X.append(I) 
    ...
    plt.figure()
    plt.title('data')
    plt.plot(hu,np.transpose(xtrain))
    #return map(np.asarray, [X,Y])
    # return X,Y
    #return map(np.asarray, [hu, xtrain, xtest, ytrain, ytest])
    return hu, xtrain, ytrain, xtest, ytest
